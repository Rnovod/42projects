...
int		ft_get_name(t_visual_data *d, char *line, char **player)
{
	char	*cp;
	int		len;
...
int			ft_get_player_name(t_visual_data *d, char **player)
{
	char	*line;
	...
	if (get_next_line(0, &line) < 0)
		ft_visual_error(d, 1);
	if (!ft_strncmp(line, "$$$ exec p1 : ", 14) || !ft_strncmp(line, "$$$ exec p2 : ", 14))
	{
...
void		ft_check_players(t_visual_data *d)
{
	...
	if (ft_strncmp("launched ", line, 9))
		ft_visual_error(d, 3);
	free(line);
	if (ft_get_player_name(d, &d->game_info.player_1))
		ft_visual_error(d, 4);
	if (get_next_line(0, &line) < 0)
		ft_visual_error(d, 1);
	if (ft_strncmp("launched ", line, 9))
		ft_visual_error(d, 3);
	free(line);
	if (ft_get_player_name(d, &d->game_info.player_2))
		ft_visual_error(d, 5);
}
