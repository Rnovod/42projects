...
int			ft_read_piece_size(t_visual_data *d, char *line)
{
	t_bytes		*ptr;
	char		*cp;
	...
	if (ptr->byte6 != 0x206563656950)
		return (-2);
	cp = line;
	if (ft_getnbr(&cp, &d->size[PIECE].h) < 0 ||
		ft_getnbr(&cp, &d->size[PIECE].w) < 0 ||
		ft_getnbr(&cp, NULL) != 0 ||
		d->size[PIECE].h <= 0 || d->size[PIECE].w <= 0)
		return (-2);
	return (0);
}

int			ft_read_piece(t_visual_data *d)
{
	int		count;
	char	*line;
